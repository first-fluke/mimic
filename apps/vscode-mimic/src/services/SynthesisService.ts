...
export class SynthesisService {
  ...
  public async checkReadiness(): Promise<{
    ...
    const threshold = vscode.workspace
      ...
      .get<number>('synthesisThreshold', 5);
   ...
   */
  public async proposeShellScript(
    selectedFiles?: string[],
  ): Promise<string | null> {
    this.outputChannel.appendLine(
      '[SynthesisService] Starting SHELL SCRIPT synthesis...',
    );
   ...
   */
  public async proposeAgentSkill(
    selectedFiles?: string[],
  ): Promise<string | null> {
    this.outputChannel.appendLine(
      '[SynthesisService] Starting AGENT SKILL synthesis...',
    );
  ...
  private async synthesize(
    ...
    if (insights.length === 0) {
      ...
      if (allInsights.length < threshold) {
        vscode.window.showWarningMessage(
          `MIMIC: Need ${threshold - allInsights.length} more insights.`,
        );
        return null;
      }
    }
    ...
    // Read files
    const insightContents = await Promise.all(
      insights.map(async (filepath: string) => {
        try {
    ...
    if (validContents.length === 0) {
      vscode.window.showWarningMessage(
        'MIMIC: No valid insight content found.',
      );
      return null;
    }
    ...
    try {
      ...
      if (result) {
        this.outputChannel.appendLine(
          '[SynthesisService] ✅ Synthesis complete.',
        );
        return result;
      }
    } catch (error) {
  ...
  public async saveSkill(
    ...
    this.outputChannel.appendLine(
      `[SynthesisService] Saved shell skill (${isGlobal ? 'Global' : 'Local'}): ${filepath}`,
    );
  ...
  public async saveAgentSkill(
    ...
    await fs.promises.writeFile(filepath, content, 'utf-8');
    this.outputChannel.appendLine(
      `[SynthesisService] Saved Agent Skill (${isGlobal ? 'Global' : 'Local'}): ${filepath}`,
    );
  ...
  private async callLLM(prompt: string): Promise<string | null> {
    try {
      ...
      if (models.length > 0) {
        ...
        this.outputChannel.appendLine(
          `[SynthesisService] Using native model: ${model.name || model.id}`,
        );
      ...
      this.outputChannel.appendLine(
        `[SynthesisService] Native LM failed: ${error}`,
      );
    }
    ...
    if (apiKey) {
      try {
        ...
        this.outputChannel.appendLine(
          `[SynthesisService] API Key fallback failed: ${error}`,
        );
      }
    }
