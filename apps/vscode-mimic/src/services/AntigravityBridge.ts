...
export class AntigravityBridge {
  ...
  public async connect(): Promise<AntigravityInfo | null> {
    ...
    this.outputChannel.appendLine(
      '[AntigravityBridge] Starting bridge connection...',
    );
    ...
    try {
      ...
      if (!csrfToken) {
        this.outputChannel.appendLine(
          '[AntigravityBridge] CSRF Token not found.',
        );
        return null;
      }
      this.outputChannel.appendLine(
        `[AntigravityBridge] CSRF Token extracted.`,
      );

      // 3. Find Ports
      const ports = await this.findPorts(pid);
      this.outputChannel.appendLine(
        `[AntigravityBridge] Found ports: ${ports.join(', ')}`,
      );

      // 4. Verify Connection (Probe all ports)
      for (const port of ports) {
        this.outputChannel.appendLine(
          `[AntigravityBridge] Probing port ${port}...`,
        );
        const isAlive = await this.probePort(port, csrfToken);
        if (isAlive) {
          this.outputChannel.appendLine(
            `[AntigravityBridge] ✅ Bridge established on port ${port}`,
          );
          this.connection = {
            pid,
            csrfToken,
      ...
      this.outputChannel.appendLine(
        `[AntigravityBridge] ❌ No working ports found.`,
      );
    } catch (e) {
      this.outputChannel.appendLine(
        `[AntigravityBridge] Connection failed: ${e}`,
      );
    }
  ...
  public async getQuota(
    connection: AntigravityInfo,
  ): Promise<QuotaSnapshot | null> {
    this.outputChannel.appendLine(
      '[AntigravityBridge] Fetching Quota (Internal RPC)...',
    );
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `[AntigravityBridge] Quota Response: ${JSON.stringify(data).substring(0, 300)}...`,
      );
      return this.parseQuota(data);
    } catch (e: any) {
      this.outputChannel.appendLine(
        `[AntigravityBridge] Quota RPC Error: ${e.message}`,
      );
    }
    return null;
  }
  ...
  private parseQuota(data: any): QuotaSnapshot {
    ...
    const models: ModelQuota[] = rawModels
      ...
      .map((m: any) => {
        ...
        this.outputChannel.appendLine(
          `[AntigravityBridge] Model: ${m.modelOrAlias?.model}, Quota: ${JSON.stringify(m.quotaInfo)}`,
        );
        ...
        this.outputChannel.appendLine(
          `[AntigravityBridge]   => Remaining: ${remainingPercent}%`,
        );
  ...
  public async sendChatMessage(
    ...
    this.outputChannel.appendLine(
      '[AntigravityBridge] Sending chat message via RPC...',
    );
    ...
    if (!targetModel) {
      ...
      if (quota) {
        ...
        if (flash) {
          ...
          this.outputChannel.appendLine(
            `[AntigravityBridge] Auto-selected Flash model: ${targetModel}`,
          );
        }
      }
    }
    ...
    const portsToTry =
      connection.availablePorts && connection.availablePorts.length > 0
    ...
    for (const port of portsToTry) {
      for (const endpoint of endpoints) {
        try {
          this.outputChannel.appendLine(
            `[AntigravityBridge] Trying port ${port}, endpoint: ${endpoint}`,
          );
          ...
          if (response) {
            this.outputChannel.appendLine(
              '[AntigravityBridge] ✅ Chat RPC Success!',
            );
    ...
    this.outputChannel.appendLine(
      '[AntigravityBridge] All chat endpoints failed.',
    );
    return null;
  }
  ...
  private async extractCsrfToken(pid: number): Promise<string | null> {
    try {
      ...
      this.outputChannel.appendLine(
        `[AntigravityBridge] Failed to read args for PID ${pid}: ${e}`,
      );
    }
    return null;
  }
