...
const MESSAGES: Record<Language, Record<string, string>> = {
  "en-US": {
    ...
    "capabilities.empty":
      "📦 *empty rattling* No powers absorbed yet. Use `mimic_evolve` to consume some patterns!",
    ...
    "evolution_ready.hint":
      "Use `mimic_evolve` to trigger evolution, or wait for auto-evolution during idle time.",
    ...
    "macro.list_empty": "📦 *silence* No macros saved yet. Use `mimic_macro_record` to create one!",
    ...
    "commit.usage_hint":
      'Use `mimic_commit({ suggestion: 1 })` or `mimic_commit({ message: "..." })`',
    "commit.no_message":
  ...
  "ko-KR": {
    ...
    "capabilities.empty":
      "📦 *텅 빈 덜컹* 아직 흡수한 능력이 없어. `mimic_evolve`로 패턴을 먹여줘!",
    ...
    "evolution_ready.hint": "`mimic_evolve`로 진화하거나, 유휴 시간에 자동 진화를 기다려.",
    ...
    "macro.list_empty": "📦 *조용* 저장된 매크로가 없어. `mimic_macro_record`로 만들어봐!",
    ...
    "commit.usage_hint":
      '`mimic_commit({ suggestion: 1 })` 또는 `mimic_commit({ message: "..." })` 사용',
