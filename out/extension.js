...
const AnalystService_1 = require("./services/AnalystService");
...
const QuickActionService_1 = require("./services/QuickActionService");
const AntigravityOAuth_1 = require("./services/AntigravityOAuth");
const AntigravityBridge_1 = require("./services/AntigravityBridge");
const InsightService_1 = require("./services/InsightService");
const SettingsPanel_1 = require("./services/SettingsPanel");
async function activate(context) {
    ...
    const installer = new Installer_1.Installer(context);
    ...
    const insightService = new InsightService_1.InsightService(outputChannel, workspaceRoot, eventsPath);
    const activityWatcher = new ActivityWatcher_1.ActivityWatcher(outputChannel, eventsPath, insightService);
    const synthesisService = new SynthesisService_1.SynthesisService(outputChannel, workspaceRoot);
    const quickActionService = new QuickActionService_1.QuickActionService(outputChannel, eventsPath);
    const antigravityOAuth = new AntigravityOAuth_1.AntigravityOAuth(context, outputChannel);
    // Wire OAuth into AnalystService for Cloud API calls
    (0, AnalystService_1.setOAuthService)(antigravityOAuth);
    // 4. Initialize Antigravity Bridge (Local RPC)
    const antigravityBridge = new AntigravityBridge_1.AntigravityBridge(outputChannel);
    (0, AnalystService_1.setBridgeService)(antigravityBridge);
    // 2. Initialize Sidebar with service references
    const sidebarProvider = new SidebarProvider_1.SidebarProvider(outputChannel, antigravityBridge, antigravityOAuth);
    ...
    const analyzeCommand = vscode.commands.registerCommand('mimic.analyzePatterns', async () => {
        await (0, AnalystService_1.analyzePatterns)(outputChannel);
    });
    ...
    const runQuickActionCommand = vscode.commands.registerCommand('mimic.runQuickAction', async (command) => {
        await quickActionService.executeCommand(command);
    });
    // Open Skill file in editor
    const openSkillCommand = vscode.commands.registerCommand('mimic.openSkill', async (skillPath) => {
        const doc = await vscode.workspace.openTextDocument(skillPath);
        await vscode.window.showTextDocument(doc);
    });
    // Install Skill to shell config
    const installSkillCommand = vscode.commands.registerCommand('mimic.installSkill', async (arg) => {
        let skillPath;
        ...
        else if (arg && arg.tooltip) {
            // Fallback if resourceUri missing
            skillPath = arg.tooltip;
        }
        if (skillPath) {
    ...
    const removeQuickActionCommand = vscode.commands.registerCommand('mimic.removeQuickAction', (item) => {
    ...
    activityWatcher.onDidDetectError(async (cmd) => {
        ...
        const selection = await vscode.window.showInformationMessage(`MIMIC: "${cmd}" failed. Analyze?`, 'Analyze', 'Ignore');
    ...
    context.subscriptions.push(analyzeCommand, viewInsightsCommand, synthesizeCommand, synthesizeAgentCommand, runQuickActionCommand, removeQuickActionCommand, addIgnorePatternCommand, openSkillCommand, installSkillCommand, setApiKeyCommand, setGoogleCredentialsCommand, loginAntigravityCommand, openSettingsCommand, switchAntigravityAccountCommand, switchAntigravityAccountCommand, refreshQuotaCommand, refreshSidebarCommand, activityWatcher, outputChannel);
...
async function handleSynthesis(service, sidebar, outputChannel, type) {
    ...
    await vscode.window.withProgress({
        ...
        const choice = await vscode.window.showInformationMessage(`MIMIC: Where should this ${type === 'shell' ? 'Shell Script' : 'Agent Skill'} be saved?`, ...options);
...
async function installSkillToZshrc(skillPath, outputChannel) {
...
async function installAgentSkill(sourcePath, outputChannel) {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
    ...
    try {
        ...
        if (fs.existsSync(targetPath)) {
            ...
            // Ask to overwrite
            const overwrite = await vscode.window.showWarningMessage(`MIMIC: Skill "${skillName}" already exists in this project. Overwrite?`, 'Overwrite', 'Cancel');
