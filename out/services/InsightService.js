...
class InsightService {
    constructor(outputChannel, workspaceRoot, eventsPath) {
        // Events
        this._onDidStartAnalysis = new vscode.EventEmitter();
        this.onDidStartAnalysis = this._onDidStartAnalysis.event;
        this._onDidEndAnalysis = new vscode.EventEmitter();
        this.onDidEndAnalysis = this._onDidEndAnalysis.event;
        ...
        this.authService = new AuthService_1.AuthService(outputChannel);
    ...
    async performAnalysis() {
        if (this.isAnalysisInProgress) {
            this.outputChannel.appendLine('[InsightService] Analysis already in progress. Skipping.');
            return;
        }
        this.isAnalysisInProgress = true;
        this._onDidStartAnalysis.fire();
        this.outputChannel.appendLine('[InsightService] Forming new insight (background)...');
        try {
            const events = await this.getRecentEvents(50);
            if (events.length < 5) {
                this.outputChannel.appendLine('[InsightService] Not enough events to analyze.');
                return;
            }
            const prompt = INSIGHT_PROMPT.replace('[EVENTS_PLACEHOLDER]', JSON.stringify(events, null, 2));
            const analysis = await this.callLLM(prompt);
            if (analysis) {
                await this.saveInsight(analysis);
                this.outputChannel.appendLine('[InsightService] ✅ New insight formed and saved.');
            }
        }
        catch (error) {
            this.outputChannel.appendLine(`[InsightService] Error forming insight: ${error}`);
        }
        finally {
            this.isAnalysisInProgress = false;
    ...
    async callLLM(prompt) {
        ...
        try {
            ...
            for (let i = 0; i < 30; i++) {
                ...
                if (i < 5 || i % 5 === 0) {
                    this.outputChannel.appendLine(`[InsightService] No models found yet. Retrying (${i + 1}/30)...`);
                }
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
            if (models.length > 0) {
                // Prioritize 'flash' models (cheapest/fastest) as requested
                const preferred = models.find(m => m.name.toLowerCase().includes('flash') || m.id.toLowerCase().includes('flash'));
                // Fallback to other Gemini/GPT-4 models
                const fallback = models.find(m => m.family.includes('gemini') || m.family.includes('gpt-4'));
                const model = preferred || fallback || models[0];
                this.outputChannel.appendLine(`[InsightService] Using native model: ${model.name || model.id} (Family: ${model.family})`);
                ...
                this.outputChannel.appendLine('[InsightService] Native LM failed: No models found after retries.');
            }
        }
        catch (error) {
            this.outputChannel.appendLine(`[InsightService] Native LM failed: ${error}`);
        }
        ...
        try {
            ...
            if (token) {
                this.outputChannel.appendLine('[InsightService] Using Google Token (Antigravity Quota)...');
            ...
            this.outputChannel.appendLine(`[InsightService] Google Login failed: ${e}`);
        }
        ...
        if (apiKey) {
            try {
                ...
                this.outputChannel.appendLine(`[InsightService] API Key fallback failed: ${error}`);
            }
        }
        this.outputChannel.appendLine('[InsightService] No LLM available. Skipping insight formation.');
        return null;
    }
    /**
     ...
     */
    async callGoogleCloudAPI(token, prompt) {
        ...
        const endpoint = 'https://daily-cloudcode-pa.googleapis.com/v1/chat/completions';
        try {
            ...
            if (response.ok) {
                ...
                this.outputChannel.appendLine(`[InsightService] Cloud API Error ${response.status}: ${response.statusText}`);
                if (response.status === 403) {
                    this.outputChannel.appendLine('[InsightService] 403 Forbidden. Your account may not be allowlisted for daily-cloudcode-pa.');
                }
            }
        }
        catch (e) {
            this.outputChannel.appendLine(`[InsightService] Cloud API Request failed: ${e}`);
        }
        return null;
    }
    /**
