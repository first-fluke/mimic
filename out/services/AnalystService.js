...
exports.setOAuthService = setOAuthService;
exports.setBridgeService = setBridgeService;
...
function setOAuthService(oauth) {
    globalOAuth = oauth;
}
function setBridgeService(bridge) {
    globalBridge = bridge;
}
/**
...
async function analyzePatterns(outputChannel) {
    ...
    if (patterns.length === 0) {
        vscode.window.showInformationMessage('MIMIC: Not enough data for analysis yet. Keep working!');
        return;
    }
    const prompt = ANALYSIS_PROMPT.replace('[PATTERNS_PLACEHOLDER]', JSON.stringify(patterns, null, 2));
...
async function handleAnalysisResult(markdown, outputChannel) {
...
async function tryAntigravityBridge(prompt, outputChannel) {
    outputChannel.appendLine('[AnalystService] Trying Antigravity Local RPC (Bridge)...');
    if (!globalBridge) {
        outputChannel.appendLine('[AnalystService] Bridge service not initialized.');
        return null;
    }
    try {
        ...
        if (!connection) {
            outputChannel.appendLine('[AnalystService] No active bridge connection. Connecting...');
            connection = await globalBridge.connect();
        }
        if (!connection) {
            outputChannel.appendLine('[AnalystService] ❌ Failed to connect to Antigravity Bridge.');
            return null;
        }
        outputChannel.appendLine('[AnalystService] ✅ Bridge connected, sending chat request...');
        const result = await globalBridge.sendChatMessage(connection, prompt);
        if (result) {
            outputChannel.appendLine('[AnalystService] ✅ Local RPC response received!');
            return result;
        }
        outputChannel.appendLine('[AnalystService] ❌ Local RPC returned no content.');
        return null;
    }
    catch (e) {
        outputChannel.appendLine(`[AnalystService] Bridge analysis failed: ${e.message}`);
        return null;
    }
}
async function tryAntigravityCloud(prompt, outputChannel) {
    outputChannel.appendLine('[AnalystService] Trying Antigravity Cloud API (OAuth)...');
    if (!globalOAuth) {
    ...
    try {
        ...
        if (!isLoggedIn) {
            outputChannel.appendLine('[AnalystService] Not logged in to Antigravity OAuth.');
            return null;
        }
        outputChannel.appendLine('[AnalystService] ✅ OAuth authenticated, calling Cloud API...');
        const result = await globalOAuth.generateContent(prompt);
        if (result) {
            outputChannel.appendLine('[AnalystService] ✅ Cloud API response received!');
            return result;
        }
        return null;
    }
    catch (e) {
        outputChannel.appendLine(`[AnalystService] Cloud API analysis failed: ${e.message}`);
    }
    return null;
}
async function tryEditorNative(prompt, outputChannel) {
    outputChannel.appendLine('[AnalystService] Trying Editor Native (vscode.lm)...');
    try {
        const allModels = await vscode.lm.selectChatModels();
        outputChannel.appendLine(`[DEBUG] Found ${allModels.length} models total.`);
        if (allModels.length === 0) {
            outputChannel.appendLine('[AnalystService] No LM models available via vscode.lm.');
            return null;
        }
        // Log available models for debugging
        allModels.forEach(m => {
            outputChannel.appendLine(`   - [${m.vendor}] ${m.family} (${m.name}) id:${m.id}`);
        });
        // Priority Strategy:
        // 1. Specific High-Quality Families (Gemini, GPT-4, Claude)
        // 2. Vendor Specific (Copilot)
        // 3. ANY available model
        const preferredFamilies = ['gemini-1.5-pro', 'gemini', 'gpt-4o', 'claude-3-5-sonnet', 'claude-3-opus'];
        // 1. Try finding a preferred model first
        let bestModel = allModels.find(m => preferredFamilies.some(f => m.family.includes(f) || m.id.includes(f)));
        ...
        if (!bestModel) {
            ...
            outputChannel.appendLine('[AnalystService] No preferred model found. Using default available model.');
        }
        if (bestModel) {
...
async function tryGoogleToken(prompt, outputChannel) {
    const authService = new AuthService_1.AuthService(outputChannel);
    const token = await authService.getToken();
    if (!token) {
        return null;
    }
    outputChannel.appendLine('[AnalystService] Trying Google Token (Native Auth)...');
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result) {
                outputChannel.appendLine('[AnalystService] Analysis complete via Gemini API.');
                const badge = `> ⚡ **Analysis by Gemini 1.5 Flash (Google Account)**\n\n---\n\n`;
                return badge + result;
            }
        }
        else {
            outputChannel.appendLine(`[AnalystService] Gemini API Error ${response.status}: ${response.statusText}`);
            if (response.status === 403) {
                outputChannel.appendLine('[AnalystService] Hint: Enabling "Generative Language API" in Google Cloud Console may be required.');
            }
        }
    }
    catch (e) {
        outputChannel.appendLine(`[AnalystService] Gemini API Request failed: ${e}`);
    }
    return null;
}
async function sendToModel(model, prompt, outputChannel) {
    outputChannel.appendLine(`[AnalystService] Sending request to model: ${model.name} (${model.id})...`);
    try {
        ...
        outputChannel.appendLine('[AnalystService] Analysis complete via Editor Native.');
        const badge = `> ⚡ **Analysis by ${model.name}**\n\n---\n\n`;
        return badge + result;
    }
    catch (err) {
        outputChannel.appendLine(`[AnalystService] Error during model request: ${err}`);
        throw err;
    }
}
async function tryCustomApiKey(prompt, outputChannel) {
    const config = vscode.workspace.getConfiguration('mimic');
    const googleKey = config.get('googleApiKey'); // Prefer Google Key
    ...
    if (googleKey) {
        ...
        try {
            ...
            outputChannel.appendLine(`[AnalystService] Gemini API failed: ${e.message}`);
        }
    }
    if (openaiKey) {
        ...
        try {
            ...
            const data = await response.json();
            outputChannel.appendLine('[AnalystService] ✅ Analysis complete via OpenAI API.');
    ...
    outputChannel.appendLine('[AnalystService] No valid custom API key configured (Google/OpenAI).');
    return null;
}
async function fallbackToClipboard(prompt, outputChannel) {
    outputChannel.appendLine('[AnalystService] Falling back to Clipboard...');
    await vscode.env.clipboard.writeText(prompt);
    vscode.window.showInformationMessage('MIMIC: 프롬프트가 복사되었습니다. AI 채팅창에 붙여넣으세요!', 'OK');
}
async function showResult(markdown) {
...
async function getRecentPatterns(workspacePath) {
    ...
    try {
        ...
        for (let i = recentLines.length - 1; i >= 0; i--) {
            ...
            try {
                ...
                stats[cmd].avgDuration = (stats[cmd].avgDuration * (stats[cmd].count - 1) + event.dur) / stats[cmd].count;
