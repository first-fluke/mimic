...
6. **NEVER do**: `@ts-ignore`, `as any` — Hiding type problems instead of solving them
...
5. **After 3 failures**: Try different approach. Record in progress
...
- **Blocked**: If no progress for 5+ turns, save current state and record `Status: blocked`
- **Scope exceeded**: If backend problem found, only record in result, do not fix directly
