...
5. **After 3 attempts**: Record `Status: cannot_reproduce` in result, specify condition list
...
6. **No progress for 5 turns**: Record current analysis results in progress, switch to different hypothesis
...
4. **Do NOT fix directly** — Touching other agent's code causes conflicts
...
Same: Refer to relevant sections in backend-agent playbook.
...
- **Scope exceeded**: For other agent's areas, only record, do not fix directly
