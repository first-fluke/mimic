...
Or create files directly in this directory when Serena is unavailable.

### Backend Agent (Implementer)
Read contract and implement exactly:
```
read_memory("api-contracts/{domain}.md")
```

### Frontend / Mobile Agent (Consumer)
Read contract and integrate API client exactly:
```
read_memory("api-contracts/{domain}.md")
```
...
4. Request re-planning from PM Agent if changes are needed
