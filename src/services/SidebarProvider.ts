...
import { AntigravityBridge, QuotaSnapshot } from './AntigravityBridge';
...
export class SidebarProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    constructor(outputChannel: vscode.OutputChannel, bridge: AntigravityBridge, oauthService?: AntigravityOAuth) {
    ...
    async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
        ...
        const quickActionLimit = config.get<number>('quickActionLimit', 10);
    ...
    private getQuotaModelItems(): Promise<vscode.TreeItem[]> {
        if (!this.quotaCache) return Promise.resolve([]);
