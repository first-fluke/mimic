...
export class QuickActionService {
    ...
    public isIgnored(cmd: string): boolean {
        ...
        if (this.ignoredPatterns.some(pattern => normalized.includes(pattern))) return true;
        return false;
    }
     ...
     */
    public getTopCommands(limit: number = 5, minCount: number = 5): CommandFrequency[] {
        ...
        for (const event of events) {
            ...
            if (this.ignoredPatterns.some(pattern => normalizedCmd.includes(pattern))) continue;
     ...
     */
    public getProjectTopCommands(workspacePath: string, limit: number = 5, minCount: number = 5): CommandFrequency[] {
        const events = this.readEvents();
        const frequency: Record<string, CommandFrequency> = {};
        ...
        for (const event of events) {
            ...
            frequency[normalizedCmd].count++;
            frequency[normalizedCmd].lastUsed = Math.max(frequency[normalizedCmd].lastUsed, (event as { ts?: number }).ts || 0);
        }
