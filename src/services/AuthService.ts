...
export class AuthService {
   ...
   */
  private async startNativeLogin(): Promise<
    vscode.AuthenticationSession | undefined
  > {
    this.outputChannel.appendLine(
      '[AuthService] Starting Native OAuth Flow...',
    );
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `[AuthService] Listening on ${redirectUri}`,
      );
      ...
      this.outputChannel.appendLine(
        '[AuthService] Browser opened. Waiting for callback...',
      );

      const code = await codePromise;
      this.outputChannel.appendLine(
        '[AuthService] Auth Code received. Exchanging for token...',
      );
  ...
  private async handleManualFallback(): Promise<
    vscode.AuthenticationSession | undefined
  > {
    const token = await vscode.window.showInputBox({
      title: 'Login Failed. Enter Access Token Manually',
      prompt:
        'Native Auth failed. Paste a valid Google Access Token to continue.',
      password: true,
      ignoreFocusOut: true,
    });
        ...
        res.on('end', () => {
          try {
            ...
            if (json.access_token) {
              ...
              this.outputChannel.appendLine(
                `[AuthService] Token Error: ${JSON.stringify(json)}`,
              );
              reject(
                new Error(
                  json.error_description || 'No access token in response',
                ),
              );
            }
          } catch (e) {
