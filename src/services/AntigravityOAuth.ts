...
// Cloud API Endpoints
const _CLOUD_API_ENDPOINT_PROD = 'https://cloudcode-pa.googleapis.com';
const _CLOUD_API_ENDPOINT_DAILY =
...
export class AntigravityOAuth {
   ...
   */
  private async getClientCredentials(): Promise<OAuthCredentials> {
    try {
      ...
      if (stored) {
        ...
        if (creds.clientId && creds.clientSecret) {
          this.outputChannel.appendLine(
            '[AntigravityOAuth] Using CUSTOM Client ID.',
          );
          return {
    ...
    this.outputChannel.appendLine(
      '[AntigravityOAuth] Using DEFAULT (Shared) Client ID.',
    );
    return {
   ...
   */
  public async setCustomCredentials(
    clientId: string,
    clientSecret: string,
  ): Promise<void> {
    ...
    this.outputChannel.appendLine(
      '[AntigravityOAuth] Custom credentials saved.',
    );
  }
   ...
   */
  public async clearCustomCredentials(): Promise<void> {
    await this.context.secrets.delete(CUSTOM_CREDENTIALS_KEY);
    this.outputChannel.appendLine(
      '[AntigravityOAuth] Custom credentials cleared. Reverted to default.',
    );
  }
   ...
   */
  public async login(forceSwitch: boolean = false): Promise<boolean> {
    this.outputChannel.appendLine(
      '[AntigravityOAuth] Starting OAuth login flow...',
    );
    ...
    this.outputChannel.appendLine(
      '[AntigravityOAuth] Opened browser for Google login...',
    );

    try {
      const code = await authCodePromise;
      this.outputChannel.appendLine(
        '[AntigravityOAuth] Received authorization code.',
      );
      ...
      if (tokens) {
        ...
        this.outputChannel.appendLine(
          `[AntigravityOAuth] ✅ Login successful! (${tokens.email})`,
        );
        vscode.window.showInformationMessage(
          `MIMIC: Logged in as ${tokens.email}`,
        );
        return true;
      }
    } catch (e: any) {
      this.outputChannel.appendLine(
        `[AntigravityOAuth] Login failed: ${e.message}`,
      );
  ...
  private startCallbackServer(): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.callbackServer = http.createServer((req, res) => {
        ...
        if (url.pathname === '/oauth-callback') {
          ...
          if (code) {
            ...
            res.end(
              '<h1>Login Successful!</h1><p>You can close this window and return to VS Code.</p>',
            );
            clearTimeout(timeout);
            this.stopCallbackServer();
            resolve(code);
      ...
      this.callbackServer.listen(51121, '127.0.0.1', () => {
        this.outputChannel.appendLine(
          '[AntigravityOAuth] Callback server listening on port 51121',
        );
      });
  ...
  private async exchangeToken(
    ...
    let _tierId: string | undefined;
    try {
      const context = await this.fetchProjectContext(data.access_token);
      projectId = context.projectId;
      _tierId = context.tierId;
    } catch (_e) {
      this.outputChannel.appendLine(
        `[AntigravityOAuth] Project context fetch failed, using defaults.`,
      );
    }
  ...
  private async fetchProjectContext(
    ...
    this.outputChannel.appendLine(
      '[AntigravityOAuth] Fetching managed project context...',
    );
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        this.outputChannel.appendLine(
          `[AntigravityOAuth] Trying loadCodeAssist: ${url}`,
        );
        ...
        if (response.ok) {
          const data = (await response.json()) as any;
          this.outputChannel.appendLine(
            `[AntigravityOAuth] loadCodeAssist success via ${endpoint}`,
          );
          this.outputChannel.appendLine(
            `[AntigravityOAuth] Payload: ${JSON.stringify(data).substring(0, 500)}`,
          );
          ...
          if (data.allowedTiers && Array.isArray(data.allowedTiers)) {
            this.outputChannel.appendLine(
              `[AntigravityOAuth] Found ${data.allowedTiers.length} allowed tiers`,
            );
            for (const tier of data.allowedTiers) {
              const tierName = tier.name?.toLowerCase() || '';
              const tierIdStr = tier.id?.toLowerCase() || '';
              this.outputChannel.appendLine(
                `[AntigravityOAuth]   Tier: ${tier.id} (${tier.name})`,
              );

              // Prefer Pro/Premium tier (Antigravity 'Pro' or similar)
              if (
                tierName.includes('pro') ||
                tierIdStr.includes('pro') ||
                tierName.includes('premium') ||
                tierIdStr.includes('premium')
              ) {
                tierId = tier.id;
                selectedTierIsPro = true;
                this.outputChannel.appendLine(
                  `[AntigravityOAuth] ✅ Selected Pro tier candidate: ${tierId}`,
                );
                break;
              }
              ...
              if (tier.isDefault && !tierId) {
                tierId = tier.id;
                this.outputChannel.appendLine(
                  `[AntigravityOAuth] Selected default tier candidate: ${tierId}`,
                );
              }
            }
          }
          ...
          if (!tierId && data.currentTier?.id) {
            ...
            this.outputChannel.appendLine(
              `[AntigravityOAuth] Using currentTier: ${tierId}`,
            );
          }
          ...
          if (selectedTierIsPro && tierId && currentTierId !== tierId) {
            this.outputChannel.appendLine(
              `[AntigravityOAuth] Current tier (${currentTierId}) != Candidate (${tierId}). Attempting onboardUser...`,
            );
            const onboardedProjectId = await this.onboardUser(
              accessToken,
              tierId,
              projectId,
            );
            if (onboardedProjectId) {
              ...
              this.outputChannel.appendLine(
                `[AntigravityOAuth] ✅ Onboard complete. Switch to: ${projectId}`,
              );
            }
          }
          ...
          if (projectId || tierId) {
            this.outputChannel.appendLine(
              `[AntigravityOAuth] ✅ Project: ${projectId}, Tier: ${tierId}`,
            );
          ...
          const errorText = await response.text();
          this.outputChannel.appendLine(
            `[AntigravityOAuth] loadCodeAssist failed (${endpoint}): ${response.status} - ${errorText.substring(0, 200)}`,
          );
        }
      } catch (e: any) {
        this.outputChannel.appendLine(
          `[AntigravityOAuth] loadCodeAssist error (${endpoint}): ${e.message}`,
        );
      }
    }

    this.outputChannel.appendLine(
      '[AntigravityOAuth] ⚠️ Failed to load project context. Using fallbacks.',
    );
    return {};
  }
  ...
  private async onboardUser(
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        this.outputChannel.appendLine(
          `[AntigravityOAuth] Calling onboardUser: ${url} (Tier: ${tierId})`,
        );
        ...
        if (response.ok) {
          ...
          if (data.done) {
            ...
            if (projectId) return projectId; // Return existing if unchanged
          }
        } else {
          this.outputChannel.appendLine(
            `[AntigravityOAuth] onboardUser failed: ${response.status}`,
          );
        }
      } catch (e: any) {
        this.outputChannel.appendLine(
          `[AntigravityOAuth] onboardUser error: ${e.message}`,
        );
      }
    }
    return undefined;
  ...
  public async getValidAccessToken(): Promise<string | null> {
    ...
    if (tokens.expiresAt < Date.now() + 300000) {
      this.outputChannel.appendLine(
        '[AntigravityOAuth] Token expired, refreshing...',
      );
      const refreshed = await this.refreshAccessToken();
      if (!refreshed) {
        this.outputChannel.appendLine(
          '[AntigravityOAuth] Token refresh failed.',
        );
        return null;
      }
  ...
  public async getProjectId(): Promise<string | null> {
    ...
    if (context.projectId) {
      ...
      const updatedTokens = tokens || ({} as any);
  ...
  public async generateContent(
    ...
    if (!accessToken) {
      this.outputChannel.appendLine(
        '[AntigravityOAuth] No valid access token.',
      );
      return null;
    }
    ...
    this.outputChannel.appendLine(
      `[AntigravityOAuth] Calling Cloud API (project: ${projectId}, tier: ${tierId}, model: ${model})...`,
    );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.candidates?.[0]?.content?.parts?.[0]?.text) {
            ...
            this.outputChannel.appendLine(
              `[AntigravityOAuth] ✅ Response received (${text.length} chars)`,
            );
            return text;
          }
          ...
          this.outputChannel.appendLine(
            `[AntigravityOAuth] Unexpected response format: ${JSON.stringify(data).substring(0, 300)}`,
          );
        } else {
          const errorText = await response.text();
          this.outputChannel.appendLine(
            `[AntigravityOAuth] API error (${response.status}): ${errorText.substring(0, 500)}`,
          );

          // Don't try other endpoints if auth failed
          if (response.status === 401 || response.status === 403) {
            this.outputChannel.appendLine(
              '[AntigravityOAuth] Auth error - may need to re-login',
            );
            return null;
          }
        ...
        this.outputChannel.appendLine(
          `[AntigravityOAuth] Endpoint ${endpoint} failed: ${e.message}`,
        );
      }
    }
