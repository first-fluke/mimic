...
export class AntigravityOAuth {
     ...
     */
    public async setCustomCredentials(clientId: string, clientSecret: string): Promise<void> {
     ...
     */
    public async buildAuthUrl(forceSwitch: boolean): Promise<{ url: string; verifier: string }> {
     ...
     */
    private async exchangeToken(code: string, verifier: string, creds: OAuthCredentials): Promise<TokenData | null> {
     ...
     */
    private async fetchProjectContext(accessToken: string): Promise<{ projectId?: string; tierId?: string }> {
     ...
     */
    private async onboardUser(accessToken: string, tierId: string, projectId?: string): Promise<string | undefined> {
        const endpoints = [
            'https://cloudcode-pa.googleapis.com',
            'https://daily-cloudcode-pa.sandbox.googleapis.com',
        ];
    ...
    public async getProjectId(): Promise<string | null> {
        ...
        if (context.projectId) {
            ...
            if (tokens) {
                await this.saveTokens({ ...tokens, projectId: context.projectId } as any);
            }
        }
     ...
     */
    public async generateContent(prompt: string, model = 'gemini-2.0-flash'): Promise<string | null> {
        const accessToken = await this.getValidAccessToken();
        if (!accessToken) {
