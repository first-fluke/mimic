...
export class Installer {
    ...
    if (enabled) {
      if (!hasCurrentHook) {
        try {
          ...
          vscode.window.showInformationMessage(
            'MIMIC: "Real-time Perception" updated/activated. Please restart your terminal.',
          );
        } catch (error) {
          vscode.window.showErrorMessage(
            `MIMIC: Failed to install hook: ${error}`,
          );
        }
      }
    } else {
      ...
      if (hasAnyMimicHook) {
        try {
          ...
          fs.writeFileSync(this.zshrcPath, newContent, 'utf-8');
          vscode.window.showInformationMessage(
            'MIMIC: "Real-time Perception" deactivated.',
          );
        } catch (error) {
          vscode.window.showErrorMessage(
            `MIMIC: Failed to remove hook: ${error}`,
          );
        }
      }
    }
  ...
  public async forceInstall(): Promise<void> {
    if (!fs.existsSync(this.zshrcPath)) {
      vscode.window.showErrorMessage(
        'MIMIC: ~/.zshrc not found. Automatic installation is only supported for Zsh.',
      );
      return;
    }
    ...
    try {
      ...
      vscode.window.showInformationMessage(
        'MIMIC: Hook installed/updated! Please restart your terminal.',
      );
    } catch (error) {
      ...
      if (fs.existsSync(gitignorePath)) {
        try {
          ...
          if (!content.includes('.mimic/')) {
            ...
            if (selection === 'Yes') {
              ...
              vscode.window.showInformationMessage(
                `MIMIC: Added .mimic/ to .gitignore in "${folder.name}"`,
              );
            }
          }
        } catch (_error) {
        ...
        if (fs.existsSync(gitDir)) {
          ...
          if (selection === 'Yes') {
            ...
            vscode.window.showInformationMessage(
              `MIMIC: Created .gitignore in "${folder.name}"`,
            );
          }
        }
      }
